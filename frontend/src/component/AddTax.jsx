...
export default function AddTax() {
        ...
        try {
            ...
            const payload = {
                ...
                tax_taxProduct_items: data.tax_taxProduct_items.map((item) => ({
                    product: Number(item.product?.value ?? item.product),
                    unit: item.unit?.value ?? item.unit,
                    par_unit: Number(item.par_unit),
                })),
            };
    ...
    return (
        <div className="flex items-center justify-center bg-transparent text-white px-4 py-16">
            <div className="w-full max-w-xl bg-black/60 rounded-2xl shadow-lg px-6 border border-black/60">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                            ...
                            <div key={field.id} className="bg-black/40 rounded-xl p-4 border border-white/10 space-y-3">
                                ...
                                <div className="relative bottom-5 mb-6">
                                    {errors.tax_taxProduct_items?.[index]?.product && (
                                        <p className="absolute top-0 left-0 right-0 text-red-500 text-lg text-center mt-1">
                                            {errors.tax_taxProduct_items?.[index]?.product.message}
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        />
                                        <div className="relative bottom-0 mb-6">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-2 mb-4 font-bold bg-orange-600 cursor-pointer hover:bg-orange-700 rounded text-white mt-2"
                    >
